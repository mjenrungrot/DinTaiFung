...
INPUT_FG = "/projects/grail/vjayaram/DinTaiFung/vivek_voice_train1_shift12.wav"
INPUT_BG = "/projects/grail/audiovisual/datasets/DinTaiFung/2mics_real_vctk_recorded/train/bg/background_shift0.wav"
...
def shift_input(args, input_data, input_position, inverse=False):
    ...
    for channel_idx in range(1, num_channels):
        ...
        if inverse:
            ...
            input_data[channel_idx] = torch.roll(input_data[channel_idx], -shift_samples)
        print("Shift: shift samples {}".format(shift_samples))
...
def get_items(args):
    ...
    """
    fg = librosa.core.load(INPUT_FG, mono=False, sr=args.sr, duration=50.0)[0]
    bg = librosa.core.load(INPUT_BG, mono=False, sr=args.sr, duration=50.0)[0]

    fg = (fg / (fg.std() / TARGET_FG_STD))[:, 30 * args.sr: 40 * args.sr]
    bg = (bg / (bg.std() / TARGET_BG_STD))[:, 30 * args.sr: 40 * args.sr]
...
def main(args):
    ...
    if args.debug:
        writing_dir = os.path.join(os.getcwd(), "real_recorded")
        print(writing_dir)
